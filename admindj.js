...
selectedProductForAdd = null;
 ...
 function selectProductForAdd(productId) {
   if (selectedProductForAdd === productId) {
     selectedProductForAdd = null;
   } else {
     selectedProductForAdd = productId;
   }
   renderProductsForAddFiltered(lastFilteredProducts);
 }

 function filterProductsForAdd() {
   const search = document.getElementById('addProductsSearch').value.toLowerCase();
   const filtered = products.filter(p => p.available && p.name.toLowerCase().includes(search));
   lastFilteredProducts = filtered;
   renderProductsForAddFiltered(filtered);
 }

 function renderProductsForAddFiltered(list) {
  const content = document.getElementById('addProductsContent');
  lastFilteredProducts = list;

  if (list.length === 0) {
    content.innerHTML = `<div class="empty-state" style="text-align:center; padding:40px; color:var(--text-muted)"><span style="font-size:48px">📦</span><div style="margin-top:12px">لا توجد منتجات متاحة</div></div>`;
    return;
  }

  content.innerHTML = list.map(p => {
    const isSelected = selectedProductForAdd === p.id;
    return `
    <div class="item-card ${isSelected ? 'selected' : ''}" onclick="selectProductForAdd(${p.id})" style="cursor:pointer; margin:0">
      <div class="admin-product-image-wrap">
        ${productImageHtml(p.image, p.name, 'admin-product-image')}
        ${isSelected ? `<div style="position:absolute; top:8px; right:8px; background:var(--primary); color:#fff; width:24px; height:24px; border-radius:50%; display:flex; align-items:center; justify-content:center; font-weight:700">✓</div>` : ''}
      </div>
      <div class="item-body" style="min-height:80px">
...
function renderProductsForAdd() {
  ...
  lastFilteredProducts = availableProducts;
  renderProductsForAddFiltered(availableProducts);
}

async function confirmAddProducts() {
  if (pendingAddProductsOrderId === null || selectedProductForAdd === null) return;
  ...
  const selectedItems = [{ name: product.name, qty: 1, price: product.price }];
  const addedTotal = product.price;
  ...
  try {
    ...
    if (res.success) {
      ...
      saveOrders(orders);
      logOrderAction("AddProducts", pendingAddProductsOrderId, null, { items: selectedItems, product: product.name });
      showToast('✅ تم إضافة المنتج للطلب', 'success');
    } else {
      showToast(`❌ ${res.error || 'تعذر إضافة المنتج'}`, 'error');
    }
  } catch (e) {
...
function renderLoyaltyStats(list) {
  ...
  const totalPoints = unique.reduce((s, c) => s + (c.points || 0), 0);
  const gold = unique.filter(c => (c.points || 0) >= 300).length;
  const silver = unique.filter(c => {
    const p = c.points || 0;
    return p >= 100 && p < 300;
  }).length;
  const bronze = unique.filter(c => (c.points || 0) < 100).length;
  ...
  const sorted = [...unique].sort((a, b) => (b.points || 0) - (a.points || 0)).slice(0, 10);
  const medals = ['🥇', '🥈', '🥉'];
...
async function approveReward(requestId) {
  try {
    ...
    if (res.success) {
      logOrderAction("ApproveReward", null, null, { requestId: requestId });
...
async function rejectReward(requestId) {
  try {
    ...
    if (res.success) {
      logOrderAction("RejectReward", null, null, { requestId: requestId });
