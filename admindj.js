...
function renderProductsForAddFiltered(list) {
  ...
  content.innerHTML = list.map(p => {
    ...
    return `
    <div class="item-card ${isSelected ? 'selected' : ''}" onclick="selectProductForAdd(${p.id})" style="cursor:pointer">
...
function selectProductForAdd(productId) {
  ...
  renderProductsForAddFiltered(products.filter(p => p.available));
}
