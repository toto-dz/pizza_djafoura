...
function renderProductsForAddFiltered(list) {
  ...
  content.innerHTML = list.map(p => `
    <div class="item-card ${selectedProductForAdd === p.id ? 'selected' : ''}" onclick="selectProductForAdd(${p.id})" style="cursor:pointer; margin-bottom:12px">
      <div class="item-body" style="padding:12px">
        ${productImageHtml(p.image, p.name, 'admin-product-image')}
        <div class="item-name" style="margin-top:8px">${p.name}</div>
        <div class="item-price" style="font-size:15px">${p.price.toLocaleString()} دج</div>
      </div>
    </div>
  `).join('');
}
