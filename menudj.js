...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.getElementById('tableLabel').textContent = `طاولة رقم ${table} 🍽️`;
...
function renderCartModal() {
  ...
  if (cart.length === 0) {
    itemsEl.innerHTML = `<div class="empty-state"><span class="emoji">🛒</span><h3>السلة فارغة</h3></div>`;
    summaryEl.innerHTML = '';
    return;
  }
...
async function placeOrder() {
  ...
  try {
    ...
    document.getElementById('orderIdDisplay').textContent = '#' + String(orderData.id).slice(-4);
    document.getElementById('tableDisplay').textContent = `طاولة رقم ${table}`;
    document.getElementById('successMsg').textContent = `ستكسب ${pts} نقطة بعد إكمال التسليم 🌟`;
    document.getElementById('trackBtn').style.display = 'flex';
    document.getElementById('trackNav').querySelector('span').textContent = 'طلبي 🔴';
    document.getElementById('pointsBadge').style.display = 'none';
...
async function openLoyalty() {
  if (!localStorage.getItem('customerPhone')) return;
  endVisitOnLoyaltyClose = false;
  const content = document.getElementById('loyaltyContent');
  
  content.innerHTML = `
    <div class="empty-state" id="loyaltyPhonePrompt">
      <span class="emoji">📱</span>
      <h3>ادخل رقم هاتفك</h3>
      <p style="margin-bottom:16px">للاطلاع على نقاطك ومكافآتك</p>
      <div class="form-group">
        <input class="form-input" id="loyaltyPhoneInput" placeholder="05xxxxxxxx" type="tel" maxlength="10">
      </div>
      <button class="btn btn-primary" onclick="fetchLoyaltyForPhone()">
        <i class="fas fa-search"></i> ابحث عن نقاطك
      </button>
    </div>
  `;
...
async function fetchLoyaltyForPhone() {
  const phoneInput = document.getElementById('loyaltyPhoneInput').value.trim();
